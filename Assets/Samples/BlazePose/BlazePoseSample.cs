...
public sealed class BlazePoseSample : MonoBehaviour
{
   ...
   // [SerializeField] RawImage debugView = null;
    ...
    [SerializeField] Text wcond;
    [SerializeField] Text scond;
    ...
    void DrawJoints(Vector4[] joints)
    {
        ...
        //GameObject child = shoes.transform.GetChild(0).gameObject;
        //child.GetComponent<modelRotate>().enabled = true;
        // Draw
        for (int i = 0; i < worldJoints.Length; i++)
        {
            ...
            if (p.w > visibilityThreshold)
            {
                ...
                if (i == 15)
                {
                    wcond.text = worldJoints[i].ToString();
                    if (shoes == null)
                    {
                        
                        Quaternion dir = new Quaternion(0, 0, 0, 0);
                        Vector3 Q2v3 = new Vector3(-90, 0, 0);
                        dir.eulerAngles = Q2v3;
                        Vector3 sdi = new Vector3(worldJoints[i].x, worldJoints[i].y, worldJoints[i].z);


                        shoes = GameObject.Instantiate(right, sdi, dir) as GameObject;
                        shoes.transform.localScale = new Vector3(0.06f, 0.06f, 0.06f);
                        Debug.Log("Yes");
                    }
                    else
                    {
                        Vector3 newsdi = new Vector3(worldJoints[i].x, worldJoints[i].y, worldJoints[i].z);
                        shoes.transform.localPosition = newsdi;

                    }

                    scond.text = shoes.transform.localPosition.ToString();


                }
            }
            else if (i == 32 && shoes != null) {
                Destroy(shoes);
            }
